/**
 * dsh-scenario 宿主端：场景 = 一组插件组合的统一开关。
 * - 注册 `scenario` 设置命名空间（active + 各场景的描述/插件清单）。
 * - 场景切换时按 `plugins` 清单一键启用/停止对应插件（Loader 条目热开关）。
 * - 提供 `/api/scenario/plugins` 路由，向配置页下发自动发现的插件目录（基础/额外）。
 * 模型选择不在此托管：不同插件组合即是不同场景，模型由用户在会话窗口按需选取。
 */
...
function apply(ctx) {
  let read = () => base
  /** 自身条目 id：reconcile 永不托管自己（否则会把场景管理插件自身停掉）。 */
  const selfId = ctx.loader?.locate?.(ctx.fiber)

  /**
   * 按场景插件清单对账 Loader 条目：当前场景绑定的第三方插件启用，其余第三方插件停用。
   * 托管范围 = 所有「额外插件」（非 cordis:/@deepseek-ai 核心、非自身）。基础插件（@deepseek-ai/*）
   * 恒启用、不动；未被任何场景引用的第三方插件在场景激活时一律停用 —— 场景是插件开关的唯一权威。
   * 直接调用 Entry.update（不经过 ctx.loader.update），避免把 disabled 状态写回
   * cordis.yml 破坏补丁层组合；每次启动 / 切换时重算。
   */
  async function reconcilePlugins() {
    ...
    const s = read()
    const active = new Set((s.scenarios[s.active]?.plugins) ?? [])
    for (const entry of loader.entries()) {
      if (entry.options.group) continue
      // 永不托管自己：按自身条目 id / 包名跳过（entry.fiber 是包装 fiber，不能用来判等）。
      if (entry.id === selfId || entry.options.name === '@lifenrir/dsh-scenario') continue
      const moduleName = entry.options.name
      if (moduleName.startsWith('cordis:')) continue
      // 基础插件（@deepseek-ai/*）是运行所需，恒启用，不纳入场景托管。
